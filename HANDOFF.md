...
### 9.45 09-04:在出貨的 exe 上驗 §9.44 的修正,以及沒有對照組會怎樣 ★

§9.44 的修正發成 FM v1.6.6。**在真的 binary 上做 A/B**:v1.6.5 的 exe 還在 GitHub 上,抓回來,各跑 40 秒閒置,用 10ms 取樣數 `nvidia-smi` 的不同 PID(每次 spawn 活約 46ms,抓得到)。
...
模型預測 v1.6.6 應該是 **12 次/分鐘**(閒置 housekeeping 5 秒一次 × TTL 1.5 秒 = 每次都 miss)。27 比它高一倍,所以八成還有別的呼叫者。

查了 `snapshot()` 的四個呼叫點:`available_vendors`(已快取)、`__init__`(一次)、housekeeping、以及 `inject.py:762` —— 最後那個在 `apply()` 裡,要有 mpv 連上才走,A/B 時沒有。**都對不上。**

**於是量對照組——完全不開任何 app:**

```
baseline  15 次/分鐘        機器上別的東西(NVIDIA 常駐服務)
```
...
| | 總計 | 扣對照組 | 換算 |
|---|---|---|---|
| v1.6.5 | 49.5/分 | **34.5/分** | 1,611 ms/分鐘 |
| v1.6.6 | 27/分 | **12/分** | 560 ms/分鐘 |

12 正好等於 housekeeping 的模型。模型預測 1,557 ms/分鐘,真實 binary 量到 1,611 —— **差 4%**。

**沒有對照組,我會把 27 當成「修了一半、還有東西沒找到」,然後去追一個不存在的呼叫者。** 這是 §5 那條「量測要有對照」的具體代價:不是量錯,是量到了別人的東西。

### 9.46 09-04:同一輪量過、判斷不動的

順著「什麼東西坐在重複計時器上」掃兩邊。

**AX 的診斷面板計時器是正確的**,不用動:1 秒一次,但 `toggle_diagnostics` 開才 `start()`、關就 `stop()`;nvidia-smi 在背景執行緒,而且有「同時只有一個在飛」的合併(註解記著 ~160ms 一次)。跟 FM 那個的差別正是**有沒有被可見性關住**。

**側欄 delegate 的繪製路徑**——`_draw_clamped_text` 每次繪製每一列都新建一個 `QTextLayout`,看起來像該快取的東西。量了(offscreen,含真實縮圖):

```
_RowDelegate.paint 整列    0.247 ms
_draw_clamped_text 單獨    0.040 ms   (16%)
14 列可見 @ 60fps          3.46 ms/frame，預算 16.7 ms
```

快取排版最多省 0.56 ms/frame。**不值得,撤回。**
...
### 9.50 09-05:發 AX v1.3.7 / FM v1.6.7,以及一個會擲骰子的關卡

#### 我在上一輪的回報裡把 AX 說錯了

上一則回報說「AX 仍然是零生產碼改動」。**錯的** —— `e7c64a7` 的掃描 `onerror` 那時候已經在 HEAD 裡了。發版前查 `git diff --stat v1.3.6..HEAD -- ax_player` 才發現 +48/-5。兩個 repo 都該發。

教訓跟這個 loop 反覆撞到的是同一條:**「有沒有改動」是可以查的,不要憑印象講。**

#### 發版關卡自己是壞的

把 `run_hidden`(FM 所有子行程的唯一出口)跟 stock `subprocess.run` 對真實 binary 逐項比對,第一次跑報 **DIFFER**。

查下去不是程式的問題:比對的是 nvidia-smi 的完整輸出,而那一行**含即時遙測**——使用率 3%、VRAM 1505 MB、功耗 41.67 W——兩次連續呼叫本來就不同。**這個關卡是在擲骰子**,而它剛好在這一次擲出了假警報。

改成比對「形狀 + 不會動的欄位」(returncode、型別、欄位數、名稱與驅動版本),連跑三次都是 ALL MATCH。

跟 §9.45 那次是同一件事的另一面:**量測要對著正確的東西**。那次是漏了對照組,這次是把雜訊當成訊號。

#### 打包版裡真的有跑到嗎

`Fluid Motion 1.6.7 start ... backend=trt` 看起來像證據,但**它不是**:`backend=trt` 只需要註冊表讀到 NVIDIA,不需要 nvidia-smi 跑起來。而這次發版的主角正是那個子行程出口在 PyInstaller 底下還能不能用。

改用直接的:數 nvidia-smi 的 spawn。

```
baseline(沒有 FM)     12 / 40s
打包版 1.6.7 執行中     18 / 40s
歸因給 FM               6 / 40s  ≈ 9 次/分鐘
```

不是 0,而且接近 `HOUSEKEEPING_IDLE` 的模型。**改寫過的 `run_hidden` 在凍結的建置裡確實會生出子行程。**

#### 驗證鏈

```
四個 suite(2 repo × 2 直譯器)      151 / 318 全綠
run_hidden vs stock subprocess.run   ALL MATCH ×3(修好關卡之後)
FM 孤兒重現                          0 個(修正前 1 個)
AX 新 exe 的 ytdlp= 行               只有新版才有的行為
FM 新 exe 自報 1.6.7                 版本在 binary 裡
打包版真的 spawn nvidia-smi          6 / 40s,不是 0
...
### 9.54 09-26:審 §9.53 沒審到的那一半 —— 「從清單移除」,兩個 release 各壞一半 ★

§9.53 的審查表對 AX `e2e67c6` 只審了 resume 驗證。同一個 commit 的另一半——播放清單改成「mpv 接受之後才提交」、網址日誌去識別化——沒有人用這份文件的方法審過。

#### 1. ★ v1.3.9:啟動後按「從清單移除」,什麼都不會發生

`remove_paths` 只回傳「剛剛移除成功的」,於是 **「mpv 拒絕」和「mpv 根本沒持有」都是空集合**,而 `remove_from_playlist` 對空集合直接 return。

第二種是**每次正常啟動之後的狀態**:`main()` 以 `reload_player=False` 還原片庫(§7 那張表「開資料夾永遠從第 1 話開始」的修法),側欄是滿的、`_loaded` 是 `[]`。播放中 F5 / 重排 / 含子資料夾重列出來的新列、播放網址之後的所有列,也是同一種。

視窗層的測試沒抓到,因為**假的 `remove_paths` 用 `set()` 代表「mpv 拒絕」**——§9.53 在 FM 找到的「假物件把缺陷本身編碼進去」,這次在 AX。新測試接的是真的 `PlayerWidget.remove_paths`,不是 lambda。

修法:回傳「mpv 播放清單裡已經沒有的」——剛移除的,**或本來就沒載入的**。只有拒絕能讓一列留下。

#### 2. ★ v1.3.8 以前:移除之後,第一次點擊就把它掃回來

把 1 修好之後才看得見的第二層。點一列而 mpv 沒持有它時,`play()` 會 `open_folder()` 重新掃描——**讀的是磁碟**,所以被移除的列回到側欄,**也被交給 mpv**,下一集就播到剛移除的那一部。`main()` 的註解把這次重掃描述成「第一次點擊付的成本」,沒有人問過它的副作用。HANDOFF 裡零命中。

改成從畫面上那份清單載入 mpv:點到的那一列一定在 `_playlist` 裡(前一個分支已經 return),而 `_playlist` 就是「上次掃描減去移除」。重掃只留作 `loadlist` 被拒絕時的後備,原本「在 library root 重掃、不重新定根」的斷言原封移到後備路徑上。

#### 在真的視窗、真的 libmpv 上驗,帶對照組

五個用內建 `mpv.exe --o=` 編出來的 20 秒片段,`LOCALAPPDATA` 導到暫存,走 啟動還原 → 移除 ep2 → 點 ep4,然後**直接問 mpv 它持有什麼**。對照組是同一個 repo、同一份 runtime,只把兩個檔換成 v1.3.9 的:

```
         側欄                     mpv 的 playlist 屬性        正在播
HEAD     ep1 ep3 ep4 ep5          ep1 ep3 ep4 ep5             ep4
v1.3.9   ep1 ep2 ep3 ep4 ep5      ep1 ep2 ep3 ep4 ep5         ep4
```

還原後兩個檔的 sha256 與 HEAD 比對一致。

這一輪改動上的六個突變全數 CAUGHT:改回 `return removed`、無視拒絕、成功算成拒絕、改回重新掃描、拿掉後備、從第 0 集開始載入。

#### 審過、沒有動的

- **§9.53 的三個 FM 修正**(`8d8634a` 播放屬性還原、`6a84126` config-dir 三種答案、`4777199` 的 JS 指令佇列)逐條推演,沒有找到缺陷。播放屬性只存記憶體(FM 被強殺後不還原)是它明寫的取捨;JS 佇列裡 Python 側每個 bridge 方法都有上界,被拒絕的命令不會毒化佇列,輪詢在下一拍自我修正。
- **`safe_url` 會把 YouTube 的 `?v=` 一起拿掉**,日誌裡只剩 `https://www.youtube.com/watch`。那一行 log 當初是為了觀察網址播放失敗加的,少了影片 id 會讓某些回報較難對應——但它與「日誌裡不留觀看紀錄」是同一個取捨的兩面,屬於擁有者的決定,不是缺陷。
- **`_loaded` 的其他消費者**:只有四個(`load_playlist`、`play_path`、`play_url`、`remove_paths`),`play_path` 的未命中本來就有 `play()` 接手。這個形狀的漏網只有 `remove_paths` 一處。
...
### 9.57 09-26：片庫在睡著的 NAS 上——UI 執行緒上的每一個檔案系統呼叫 ★

§9.47 把「掃到一半閃斷的網路磁碟」列為 `onerror` 的觸發原因之一，但只看了**背景執行緒**上的掃描。這一輪問的是另一半：**UI 執行緒上，有哪些呼叫會碰到片庫所在的分享？**

#### 先量：一個不回應的分享要多久才失敗

```
不回應的位址（NAS 關機的形狀）  第一個碰到它的呼叫     21.0 s   （之後的呼叫吃到負快取）
解析不到的主機名稱                                      1.3 s
```

「第一個」很重要：探針裡 `resolve()` 排在 `is_dir()` 後面、顯示 0.00 秒，看起來無害——在全新的行程裡單獨量，它一樣是 21 秒。**排序造成的假陰性**，差點把一個有問題的呼叫判成沒問題。

#### 四個地方，一個形狀

| 位置 | 什麼時候 | 修法 |
|---|---|---|
| `main()` 還原片庫：`Path(last).is_dir()` | **每次啟動**，在 `app.exec()` 之前 | `restore_library()`：daemon 執行緒問，問到了以訊號回 UI 執行緒開資料夾 |
| `open_folder()` 開頭的 `resolve()` | F5、重排、含子資料夾 | 重開**目前的**資料夾時直接沿用 `self._folder`（它第一次打開時就解析過了） |
| `play()` 開頭的 `resolve()` | 每次點列 | 已列出的列用**清單裡的那個物件**；未列出的照舊解析 |
| `reveal_in_explorer()` 的 `exists()` | 右鍵「在檔案總管中顯示」 | 整段移到 daemon 執行緒 |

用真的 `main()`、真的視窗與 libmpv 量「事件迴圈第一次跑起來」（`LOCALAPPDATA` 導到暫存、`last_folder` 設成不回應的 UNC 路徑；對照組是同一個 repo 只換回 v1.3.10 的 `app.py`）：

```
v1.3.10   19.07 s   視窗在畫面上「沒有回應」，之後顯示空側欄
改後       0.69 s   行程也在 2.4 s 正常結束——背景檢查還卡著也不擋離開
```

正面路徑同樣驗過：存在的片庫在背景確認後照常還原（5 個檔、側欄 5 列），不交給 mpv。

#### 設計上的三個選擇（理由都寫進了註解）

- **不用掃描的 pool**：它一次只跑一個工作，使用者在那 21 秒內開的資料夾會排在逾時後面。
- **不用任何 QThreadPool**：§9.48 量過 pool 在銷毀時會等工作做完——在那 21 秒內關掉 app，視窗會立刻隱藏、行程卻活到分享逾時。daemon 執行緒不擋離開。
- **沿用正規拼法，而不是呼叫端的拼法**：Windows 的 `Path` 比較不分大小寫。`open_folder` 與 `play()` 若沿用傳進來的物件，§7 表格記過的「大小寫不一致讓每次點擊都重掃、含子資料夾時還會重新定根」就會回來。測試故意用大寫拼法傳入。

另一個守衛：遲來的「片庫還在」**不可蓋掉**使用者在等待期間自己開的資料夾。

#### 測試與突變

8 條新測試（AX 184 → 192），12 個突變全部只打紅對應的那一題：`main()` 改回在 UI 執行緒上 `is_dir`（AST 檢查還原分支不可呼叫 `is_dir`/`exists`/`resolve`/`is_file`/`stat`/`open_folder`）、改成同步執行、拿掉「已開別的資料夾」守衛、還原時交給 mpv 播放、`OSError` 漏出去、`open_folder`/`play()` 各自改回一律解析、各自改成沿用呼叫端拼法、`reveal` 改成參數列表、改回 UI 執行緒、拿掉存在檢查。

`reveal_in_explorer` 原本**完全沒有測試**，連它註解裡記著的規則都沒被釘住：`/select,` 必須跟路徑黏在同一個命令字串裡，改成參數列表會被 `list2cmdline` 引號成一個 token，檔案總管就開「文件」。

真視窗、真 libmpv 的端到端流程（§9.54 的 啟動還原 → 移除 → 點列）在改動後重跑：側欄與 mpv 的播放清單一致、正在播點到的那一集。
